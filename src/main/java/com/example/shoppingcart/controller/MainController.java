...
import com.example.shoppingcart.dao.CustomerDAO;
import com.example.shoppingcart.dao.OrderDAO;
import com.example.shoppingcart.dao.ProductDAO;
import com.example.shoppingcart.dao.WishListDAO;
...
public class MainController {
	...
	public String listProductHandler(Model model, //
		...
		props.put("previousPage", page - 1);
		props.put("nextPage", page + 1);
	...
	public String product(Model model, @RequestParam(value = "id", defaultValue = "") String id) {
		CustomerValidateForm customerValidateForm = null;

		if (id != null && id.length() > 0) {
			Customer customer = customerDAO.findCustomer(id);
			if (customer != null) {
				customerValidateForm = new CustomerValidateForm(customer);
			}
		}
		if (customerValidateForm == null) {
			customerValidateForm = new CustomerValidateForm();
			customerValidateForm.setNewProduct(true);
		}
		model.addAttribute("customerForm", customerValidateForm);
	...
	public String CustomerSave(Model model, //
			@ModelAttribute("customerForm") @Validated CustomerValidateForm customerValidateForm, //
			BindingResult result, //
			final RedirectAttributes redirectAttributes) {
		...
		try {
			accountDAO.save(customerValidateForm);
			customerDAO.save(customerValidateForm);
		} catch (Exception e) {
	...
	public String addWishList(HttpServletRequest request, Model model, //
		...
		System.out.println(
				"---------- code:" + wishList.getProduct().getCode() + "Customer:" + wishList.getCustomer().getName());
		try {
	...
	public String productDetails(Model model, @RequestParam(value = "code", defaultValue = "") String code) {
		if (code != null && code.length() > 0) {
			System.out.println("---------- code:" + code);
